...
| 2 | Go version `1.25` → `1.24` in `run-lint.yml`, `run-tests.yml`, and `go.mod` | Go 1.25 does not exist; golangci-lint is built with Go 1.24 and fails when the targeted version is higher |
